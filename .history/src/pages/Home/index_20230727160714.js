...
function Home() {
    const wrap_banerRef = useRef(null);
    const [h, setH] = useState("");
  
    useEffect(() => {
      if (wrap_banerRef.current) {
        // Lấy class name từ phần tử có className là "huu"
        const className = wrap_banerRef.current.className;
        setH(className);
      }
    }, []);
    console.log(h)
    return (
        <div className={s('wrap_baner', 'p-0')}>
            <div ref={wrap_banerRef} className={s('wrap_header')}>
